...
double signedIntervalViolation(double value, double low, double high)
{
  if (value < low) {
    return value - low;
  }
...
private:
  ...
  void declareParameters()
  {
    ...
    declare_parameter("depth_proxy_m", 0.055);
    declare_parameter("standoff_desired_m", 0.12);
    ...
    declare_parameter("infeasible_comfort_weight", 0.08);
    declare_parameter("infeasible_hard_progress_epsilon", 1.0e-6);
    ...
    declare_parameter("min_step_rad", 0.001);
  ...
  void loadParameters()
  {
    ...
    feasibility_tie_break_epsilon_ =
      std::max(0.0, get_parameter("feasibility_tie_break_epsilon").as_double());
    infeasible_comfort_weight_ =
      std::max(0.0, get_parameter("infeasible_comfort_weight").as_double());
    infeasible_hard_progress_epsilon_ =
      std::max(0.0, get_parameter("infeasible_hard_progress_epsilon").as_double());
    ...
    const std::vector<double> & q0)
  {
    ...
    for (std::size_t i = 0; i < seeds.size(); ++i) {
      ...
      candidate.score = evaluate(
        ...
        effector_to_camera,
        current_state);
      considerCandidate(candidate, result);
      ...
      while (iter < max_iterations_per_seed_ && step >= min_step_rad_) {
        ...
        for (std::size_t joint_idx = 0; joint_idx < candidate.q.size(); ++joint_idx) {
          ...
          const bool plus_better = candidateBetterForSearch(plus_score, candidate.score);
          const bool minus_better = candidateBetterForSearch(minus_score, candidate.score);

          if (plus_better && (!minus_better || candidateBetterForSearch(plus_score, minus_score))) {
            candidate.q = std::move(trial_plus);
            candidate.score = plus_score;
            candidate.iterations = iter + 1;
            considerCandidate(candidate, result);
            improved = true;
          } else if (minus_better) {
  ...
  void considerCandidate(const Candidate & candidate, OptimizationResult & result) const
  {
    if (candidateBetterForBestOverall(candidate.score, result.best_overall.score)) {
      result.best_overall = candidate;
    }
    if (constraintsSatisfied(candidate.score)) {
      if (!result.best_feasible ||
        candidateBetterForBestFeasible(candidate.score, result.best_feasible->score))
      {
        result.best_feasible = candidate;
      }
      rememberFeasibleCandidate(candidate, result);
    }
  }
  ...
  void rememberFeasibleCandidate(const Candidate & candidate, OptimizationResult & result) const
  {
    ...
    for (const auto & existing : result.feasible_candidates) {
      if (sameJointTarget(candidate.q, existing.q)) {
        return;
      }
    }

    result.feasible_candidates.push_back(candidate);
    std::sort(
      result.feasible_candidates.begin(),
  ...
  bool sameJointTarget(const std::vector<double> & a, const std::vector<double> & b) const
  {
    if (a.size() != b.size()) {
      return false;
    }
    ...
    for (std::size_t i = 0; i < a.size(); ++i) {
      max_delta = std::max(max_delta, std::abs(wrapAngleDelta(a[i] - b[i])));
    }
    return max_delta < 1e-6;
  }

  bool candidateBetterForSearch(const Score & trial, const Score & current) const
  {
    if (!enable_feasibility_first_) {
      return trial.total + 1e-12 < current.total;
    }
    if (!trial.finite) {
      return false;
    }
    ...
    if (!current_feasible) {
      ...
      if (hard_progress > infeasible_hard_progress_epsilon_) {
        return true;
      }
      if (hard_progress < -feasibility_epsilon_) {
        return false;
      }
  ...
  bool candidateBetterForBestOverall(const Score & trial, const Score & current) const
  {
    ...
    if (trial.hard_violation_total + infeasible_hard_progress_epsilon_ <
      current.hard_violation_total)
    {
      return true;
    }
    if (std::abs(trial.hard_violation_total - current.hard_violation_total) <=
      feasibility_tie_break_epsilon_)
    {
      return trial.feasibility_merit < current.feasibility_merit;
    }
    return false;
  }

  bool candidateBetterForBestFeasible(const Score & trial, const Score & current) const
  {
    if (!enable_feasibility_first_) {
      return trial.total < current.total;
    }
    ...
    const moveit::core::RobotState & template_state) const
  {
    ...
    const Eigen::Vector3d ideal_camera_direction = -view_axis.normalized();
    const Eigen::Vector3d center_to_camera = p_cam - measurement.center;
    ...
    score.axial_standoff = center_to_camera.dot(ideal_camera_direction);
    const Eigen::Vector3d lateral_offset =
      center_to_camera - score.axial_standoff * ideal_camera_direction;
    score.lateral_error = lateral_offset.norm();
    score.gaze_error = angleBetween(score.camera_z, to_bottom.normalized());
    score.axis_error = angleBetween(score.camera_z, view_axis);
    score.standoff_error = signedIntervalViolation(
      score.axial_standoff, standoff_min_m_, standoff_max_m_);
  ...
  double axisViolationScale() const
  {
    return std::max(0.05, std::abs(axis_max_rad_));
  }

  double gazeViolationScale() const
  {
    return std::max(0.05, std::abs(gaze_max_rad_));
  }

  double lateralViolationScale() const
  {
    return std::max(0.005, lateral_max_m_);
  }

  double standoffViolationScale() const
  {
  ...
  void computeConstraintViolation(Score & score) const
  {
    ...
    score.axis_violation_norm = enforce_axis_max_ ?
      std::max(0.0, score.axis_error - axis_max_rad_) / axisViolationScale() : 0.0;
    score.lateral_violation_norm = enforce_lateral_max_ && lateral_max_m_ > 0.0 ?
      std::max(0.0, score.lateral_error - lateral_max_m_) / lateralViolationScale() : 0.0;
    score.standoff_min_violation_norm =
      std::max(0.0, standoff_min_m_ - score.axial_standoff) / standoffViolationScale();
    score.standoff_max_violation_norm =
    ...
    score.hard_violation_total =
      ...
      square(score.lateral_violation_norm) +
  ...
  {
    const bool metrics_finite =
      std::isfinite(score.motion_cost) &&
      std::isfinite(score.wrist_cost) &&
    ...
    double violation) const
  {
    ...
    out << "{"
        ...
        << "\"violation\":" << jsonNumber(violation)
        << "}";
  }

  std::string constraintFailuresJson(const Score & score) const
  {
    std::ostringstream out;
    out << '[';
    bool first = true;
    ...
    if (!score.finite) {
      appendConstraintFailure(
        out, first, "finite_score", score.total, 0.0,
        std::numeric_limits<double>::infinity());
    }
    if (score.axial_standoff < standoff_min_m_) {
      appendConstraintFailure(
        out, first, "axial_standoff_min", score.axial_standoff, standoff_min_m_,
        standoff_min_m_ - score.axial_standoff);
    }
    if (score.axial_standoff > standoff_max_m_) {
      appendConstraintFailure(
        out, first, "axial_standoff_max", score.axial_standoff, standoff_max_m_,
        score.axial_standoff - standoff_max_m_);
    }
    if (enforce_lateral_max_ && lateral_max_m_ > 0.0 && score.lateral_error > lateral_max_m_) {
      appendConstraintFailure(
    ...
    bool published)
  {
    ...
    out << "{"
        ...
        << jsonNumber(best.score.standoff_min_violation_norm) << ','
        ...
        << "\"standoff_reference\":\"axial_standoff_interval\","
        << "\"standoff_desired_m\":" << jsonNumber(standoff_desired_m_) << ','
        << "\"standoff_interval_m\":[" << jsonNumber(standoff_min_m_) << ','
        << jsonNumber(standoff_max_m_) << "],"
        ...
        << "\"selected_reason\":" << jsonString(selected_reason)
        << "}";
    msg.data = out.str();
    debug_pub_->publish(msg);
  }
  ...
  double standoff_desired_m_ {0.12};
  ...
  double infeasible_comfort_weight_ {0.08};
  double infeasible_hard_progress_epsilon_ {1.0e-6};
