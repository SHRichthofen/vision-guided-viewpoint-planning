...
class RGBCylinderPoseEstimator:
    ...
    def _fit_side_line_from_points(self, points_xy, body_dir, signed_offset, label):
        pts = np.asarray(points_xy, dtype=np.float32).reshape(-1, 2)
        if len(pts) < cfg.BODY_SIDE_MIN_POINTS:
            return None
        ...
        if np.linalg.norm(line_dir) < 1e-6:
            return None
        line_dir /= (np.linalg.norm(line_dir) + 1e-6)
        if float(np.dot(line_dir, body_dir)) < 0.0:
            line_dir = -line_dir

        p0 = np.array([float(x0), float(y0)], dtype=np.float32)
        t_vals = (pts - p0) @ line_dir
        t_min = float(np.min(t_vals))
        t_max = float(np.max(t_vals))
        seg_p0 = p0 + line_dir * t_min
        seg_p1 = p0 + line_dir * t_max
        normal = np.array([-line_dir[1], line_dir[0]], dtype=np.float32)
        residuals = np.abs((pts - p0) @ normal)
    ...
    def detect_body_side_lines(self, body_mask, body_bbox=None):
        ...
        if body_geom is None:
            ...
            body_geom = self._body_bbox_geometry((x + w * 0.5, y + h * 0.5, w, h))
        if body_geom is None:
            return []

        body_dir = body_geom['body_dir']
        body_center = body_geom['center_uv']
        short_span = max(6.0, min(body_geom['bw'], body_geom['bh']))
        long_span = max(body_geom['bw'], body_geom['bh'])
        normal_to_body = np.array([-body_dir[1], body_dir[0]], dtype=np.float32)

        signed_offsets = (contour_pts - body_center) @ normal_to_body
        q = float(np.clip(cfg.BODY_SIDE_POINT_QUANTILE, 0.05, 0.45))
        q_low = float(np.quantile(signed_offsets, q))
        q_high = float(np.quantile(signed_offsets, 1.0 - q))

        neg_pts = contour_pts[signed_offsets <= q_low]
        pos_pts = contour_pts[signed_offsets >= q_high]
        if len(neg_pts) < cfg.BODY_SIDE_MIN_POINTS or len(pos_pts) < cfg.BODY_SIDE_MIN_POINTS:
            return []

        neg_line = self._fit_side_line_from_points(
            neg_pts,
            body_dir,
            float(np.mean(signed_offsets[signed_offsets <= q_low])),
            'S0'
        )
        pos_line = self._fit_side_line_from_points(
            pos_pts,
            body_dir,
            float(np.mean(signed_offsets[signed_offsets >= q_high])),
            'S1'
        )
        side_lines = [line for line in (neg_line, pos_line) if line is not None]
        if len(side_lines) < 2:
            return []

        valid_lines = []
        for line in side_lines:
            body_alignment = abs(float(np.dot(line['direction'], body_dir)))
            line['body_alignment'] = body_alignment
            line['span_ratio'] = float(line['length_px'] / (long_span + 1e-6))
            if body_alignment < 0.75:
                continue
        ...
        valid_lines = sorted(valid_lines, key=lambda item: item['signed_offset'])
        separation = abs(valid_lines[-1]['signed_offset'] - valid_lines[0]['signed_offset'])
        if separation < cfg.BODY_SIDE_MIN_SEPARATION_RATIO * short_span:
            return []
    ...
    def process_detection(
        ...
        side_lines = self.detect_body_side_lines(roi_body_mask, body_bbox=local_body_bbox)
