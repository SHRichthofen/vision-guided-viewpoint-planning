...
class CylinderDetectionNode(Node):
    ...
    def run_rim_pipeline(self, img, target_tube, depth_prior_m=None, depth_prior_confidence=0.0):
        ...
        adaptive_dilation = int(max(
            cfg.RIM_MASK_DILATION_MIN,
            min(cfg.RIM_MASK_DILATION_MAX, bbox_w * cfg.RIM_MASK_DILATION_SCALE)
        ))
        kernel = np.ones((adaptive_dilation, adaptive_dilation), np.uint8)
        dilated_mask = cv2.dilate(best_rim_mask, kernel, iterations=1)

        global_rim_mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
        global_rim_mask[cy1:cy2, cx1:cx2] = dilated_mask
