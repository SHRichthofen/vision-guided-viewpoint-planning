...
class RGBCylinderPoseEstimator:
    ...
    def solve_circle_pose_candidates(self, pose_result):
        """为第二阶段预留单圆双解接口；本阶段先暴露统一的数据结构。"""
        return [{
            'candidate_id': 0,
            'center_3d': [float(x) for x in pose_result['center_3d']],
            'axis_3d': [float(x) for x in pose_result['normal']],
            'solver_stage': 'phase1_placeholder',
            'note': 'single-circle dual-solution is reserved for phase2',
        }]

    def frontal_fallback(self, pose_result):
        """为后续 near-circle 专门分支预留接口，本阶段只打标不重写几何。"""
    ...
    def _estimate_pose_from_ellipse(self, local_ellipse, roi_offset, depth_prior_m=None, depth_prior_confidence=0.0):
        ...
        e_x, e_y = local_ellipse[0]
        real_angle = local_ellipse[2]
        global_cx = e_x + roi_offset[0]
        global_cy = e_y + roi_offset[1]
        ...
        phi = math.radians(real_angle + 90.0)
        ...
        pose_result = {
            ...
            'observability_state': 'near_circle' if near_circle else 'oblique',
        }
        if near_circle:
    ...
    def process_detection(
        ...
        pose_result, status = self._estimate_pose_from_ellipse(
            final_ellipse,
            (x1, y1),
        ...
        pose_result['rim_pipeline_stage'] = 'phase1_candidate_pipeline'
        ...
        pose_result['pose_candidates_3d'] = self.solve_circle_pose_candidates(pose_result)
