...
class CylinderDetectionNode(Node):
    ...
    def _build_selected_semantics_payload(self, track_id, pose):
        ...
        if 'side_oblique_ready' in pose:
            payload['side_oblique_ready'] = bool(pose['side_oblique_ready'])
        if 'side_line_candidate_count' in pose:
            payload['side_line_candidate_count'] = int(pose['side_line_candidate_count'])
        if 'side_line_pair_metrics' in pose:
            payload['side_line_pair_metrics'] = {
                k: float(v) for k, v in pose['side_line_pair_metrics'].items()
            }
        if 'body_axis_source' in pose:
            payload['body_axis_source'] = str(pose['body_axis_source'])
    ...
    def run_rim_pipeline(self, img, target_tube, depth_prior_m=None, depth_prior_confidence=0.0):
        ...
        return raw_pose, global_rim_mask, roi_vis, (cx1, cy1)
