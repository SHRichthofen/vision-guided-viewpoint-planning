...
BODY_SIDE_EDGE_QUANTILE = 0.08        # body 主法向两侧边界估计分位数
BODY_SIDE_AXIS_TRIM_RATIO = 0.14      # 轴向两端裁剪比例，避免端盖弧边干扰
BODY_SIDE_BAND_WIDTH_MIN = 4          # 左右搜索带最小半宽
BODY_SIDE_BAND_WIDTH_MAX = 14         # 左右搜索带最大半宽
BODY_SIDE_BAND_WIDTH_RATIO = 0.18     # 搜索带半宽相对 body 短边比例
BODY_SIDE_INNER_ALLOWANCE_MIN = 2     # 搜索带允许进入 mask 内部的最小像素
BODY_SIDE_INNER_ALLOWANCE_MAX = 8     # 搜索带允许进入 mask 内部的最大像素
BODY_SIDE_INNER_ALLOWANCE_RATIO = 0.16  # 搜索带允许进入 mask 内部的相对比例
BODY_SIDE_GRADIENT_PERCENTILE = 72.0  # 搜索带内法向梯度阈值分位数
BODY_SIDE_GRADIENT_MIN = 12.0         # 搜索带内法向梯度最低阈值
BODY_SIDE_BIN_SIZE_PX = 3.0           # 沿主轴聚合支持点的分箱尺寸
BODY_SIDE_SUPPORT_MIN_POINTS = 10     # 每侧精修支持点最小数量
BODY_SIDE_SUPPORT_MIN_BINS = 8        # 每侧支持点沿主轴最少覆盖分箱数
BODY_SIDE_MIN_BODY_ALIGNMENT = 0.82   # 单侧边与 body 主轴最小夹角一致性
BODY_SIDE_PAIR_PARALLEL_MIN = 0.94    # 双侧边最小平行性
BODY_SIDE_PAIR_SUPPORT_BALANCE_MIN = 0.45  # 左右支持点数量平衡下限
BODY_SIDE_OBLIQUE_MIN_SCORE = 0.62    # 进入 oblique 前的最小综合质量分
...
ROI_DEBUG_SHOW_SIDE_DIAGNOSTICS = True    # ROI调试图默认显示侧边搜索带/支持点/候选线
