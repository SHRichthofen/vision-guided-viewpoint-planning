...
class RGBCylinderPoseEstimator:
    ...
    def score_candidates_rgb_only(self, pose_candidates, body_bbox=None):
        """第二阶段基础候选评分：只使用 rim/body 的 RGB 几何，不引入侧边母线。"""
    ...
    def disambiguate_with_side_lines(self, pose_candidates, side_lines, side_diag=None, body_bbox=None):
        """用侧边母线的几何方向直接消除单圆双解，不参与 rim/candidate 质量评分。"""
        if not pose_candidates:
            return [], None, {
                'applied': False,
                'reason': 'no pose candidates',
            }

        base_best = dict(pose_candidates[0])
        side_context = self._build_side_line_pose_context(side_lines, side_diag)
        if side_context is None:
            return pose_candidates, base_best, {
                'applied': False,
                'reason': 'side lines unavailable or rejected',
            }
        if base_best.get('near_circle_degenerate', False):
            return pose_candidates, base_best, {
                'applied': False,
                'reason': 'near-circle frontal mode skips side-line disambiguation',
            }
        ...
        for candidate in pose_candidates:
            cand = dict(candidate)
            axis_img_dir = np.asarray(cand.get('axis_img_dir', side_context['axis_dir']), dtype=np.float32)
            axis_norm = float(np.linalg.norm(axis_img_dir))
            if axis_norm < 1e-6:
                cand['side_disambiguation_score'] = 0.0
                cand['side_axis_alignment_score'] = 0.0
                cand['side_body_direction_score'] = 0.0
                scored.append(cand)
                continue
            axis_img_dir /= axis_norm
            ...
            axis_alignment_score = float(np.clip(
            ...
            if target_norm > 1e-6:
                ...
                body_direction_score = float(np.clip(
                    0.5 + 0.5 * float(np.dot(axis_img_dir, target_dir)),
                    0.0,
                    1.0
                ))
            else:
                body_direction_score = 0.5

            disambiguation_score = float(np.clip(
                cfg.SIDE_DISAMBIG_AXIS_WEIGHT * axis_alignment_score
                + cfg.SIDE_DISAMBIG_BODY_DIRECTION_WEIGHT * body_direction_score,
                0.0,
                1.0
            ))

            cand['side_disambiguation_score'] = disambiguation_score
            cand['side_axis_alignment_score'] = axis_alignment_score
            cand['side_body_direction_score'] = body_direction_score
            cand['side_line_quality'] = float(side_context.get('quality', 0.0))
            scored.append(cand)

        side_sorted = sorted(
            scored,
            key=lambda item: item.get('side_disambiguation_score', 0.0),
            reverse=True
        )
        selected = dict(side_sorted[0])
        runner_up = side_sorted[1] if len(side_sorted) > 1 else None
        margin = float(
            selected.get('side_disambiguation_score', 0.0)
            - (runner_up.get('side_disambiguation_score', 0.0) if runner_up is not None else 0.0)
        )
        selected['side_disambiguation_applied'] = True
        selected['rgb_only_reason'] = (
            f"side geometric disambiguation P{int(selected.get('candidate_id', -1))}, "
            f"side={float(selected.get('side_disambiguation_score', 0.0)):.2f}, "
            f"margin={margin:.2f}, base_rgb={float(selected.get('rgb_only_score', 0.0)):.2f}"
        )

        ordered = []
        selected_id = int(selected.get('candidate_id', -1))
        for cand in scored:
        ...
        return ordered, selected, {
            ...
            'score': float(selected.get('side_disambiguation_score', 0.0)),
            'margin': margin,
            'side_line_quality': float(side_context.get('quality', 0.0)),
            'reason': selected['rgb_only_reason'],
        }
    ...
    def _overlay_pose_debug_text(self, debug_vis_img, pose_result):
        ...
        for cand in pose_candidates:
            if int(cand.get('candidate_id', -1)) == selected_id:
                ...
                break
        if selected_pose_debug is not None and bool(selected_pose_debug.get('side_disambiguation_applied', False)):
            lines.append(
                f"SideDisamb S={float(selected_pose_debug.get('side_disambiguation_score', 0.0)):.2f}  "
                f"Ax={float(selected_pose_debug.get('side_axis_alignment_score', 0.0)):.2f}  "
                f"Tow={float(selected_pose_debug.get('side_body_direction_score', 0.0)):.2f}"
            )
    ...
    def process_detection(
        ...
        pose_candidates_scored, _ = self.score_candidates_rgb_only(
            pose_candidates,
            body_bbox=body_bbox
        )
        pose_candidates_scored, best_pose_candidate, side_disambig = self.disambiguate_with_side_lines(
            pose_candidates_scored,
            side_lines,
            side_diag=side_diag,
            body_bbox=body_bbox
        )
        ...
        pose_result['side_disambiguation'] = {
            'applied': bool(side_disambig.get('applied', False)),
            'selected_candidate_id': int(side_disambig.get('selected_candidate_id', -1)),
            'score': float(side_disambig.get('score', 0.0)),
            'margin': float(side_disambig.get('margin', 0.0)),
            'side_line_quality': float(side_disambig.get('side_line_quality', 0.0)),
            'reason': str(side_disambig.get('reason', '')),
        }
        ...
        pose_result['pose_candidates_3d'] = [
            {
                ...
                'side_disambiguation_score': float(cand.get('side_disambiguation_score', 0.0)),
                'side_axis_alignment_score': float(cand.get('side_axis_alignment_score', 0.0)),
                'side_body_direction_score': float(cand.get('side_body_direction_score', 0.0)),
                'side_disambiguation_applied': bool(cand.get('side_disambiguation_applied', False)),
