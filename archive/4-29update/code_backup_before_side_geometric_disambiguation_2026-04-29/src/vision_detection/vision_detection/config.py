...
POSE_SIDE_LINE_SCORE_WEIGHT = 0.45    # 侧边可信时，混入候选位姿评分的最大权重
POSE_SIDE_AXIS_WEIGHT = 0.55          # 侧边评分中，候选轴向与侧边方向一致性权重
POSE_SIDE_BODY_DIRECTION_WEIGHT = 0.30 # 侧边评分中，候选轴向指向管身方向权重
POSE_SIDE_CENTERLINE_WEIGHT = 0.15    # 侧边评分中，候选中心位于两侧边中线的权重
POSE_BODY_AXIS_ASPECT_MIN = 1.12      # body bbox 细长度低于此值时，方向约束置信度降低
BODY_SIDE_MIN_POINTS = 28             # body mask 侧边拟合所需最少点数
BODY_SIDE_POINT_QUANTILE = 0.18       # 两侧极值点分位数
BODY_SIDE_AXIS_TRIM_RATIO = 0.16      # 沿 body 主轴裁掉端盖区域比例
BODY_SIDE_MIN_SPAN_RATIO = 0.20       # 侧边沿 body 主轴最小覆盖比例
BODY_SIDE_MIN_SEPARATION_RATIO = 0.18 # 左右侧边最小分离比例
BODY_SIDE_MIN_BODY_ALIGNMENT = 0.80   # 侧边与 body 主轴最小一致性
BODY_SIDE_PAIR_PARALLEL_MIN = 0.92    # 左右侧边最小平行性
