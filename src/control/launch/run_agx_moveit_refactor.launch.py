...
def generate_launch_description():
    ...
    view_goal_solver_params_path = os.path.join(control_share, 'config', 'view_goal_solver.params.yaml')
    joint_states_topic = PythonExpression([
        "'feedback/joint_states' if '", follow, "' == 'true' else 'control/joint_states'"
    ])
    moveit_client_remappings = [('joint_states', joint_states_topic)]
    ...
    vision_moveit_executor_node = Node(
        ...
        remappings=moveit_client_remappings,
        condition=IfCondition(enable_executor),
    )

    view_goal_solver_node = Node(
        package='control',
        executable='view_goal_solver',
        namespace=namespace,
        output='screen',
        parameters=[view_goal_solver_params_path],
        remappings=moveit_client_remappings,
        condition=IfCondition(enable_view_goal_solver),
    )
